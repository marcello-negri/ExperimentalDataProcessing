...
double VettoreDati::Rho(VettoreDati * y){
	double appo=0;
	if (m_N!=y->GetN()){
		cout << endl << "Errore: i due vettori non sono lunghi uguali!!!" << endl << endl;
		return -99999999;
	}
	for (unsigned int i=0; i<m_N; i++){
		appo+=((m_v[i]-this->Media())*(y->GetComponent(i)-y->Media()))/m_N;
	}
	return appo/(this->DevStPop()*y->DevStPop());
}
...
VettoreLineare VettoreLineare::operator+(const VettoreLineare& v) const{
	...
	if (GetN() != v.GetN()){
		cerr << "Errore: i due vettori non sono lunghi uguali!!!" << endl << endl;
		exit (-2);
	}
...
double VettoreLineare::operator*(const VettoreLineare& v) const{
	...
	for (unsigned int i = 0; i<GetN(); i++)
		 somma+= GetComponent(i)*v.GetComponent(i);
	return somma;
}
...
VettoreLineare VettoreLineare::operator*(const double a) const{
	VettoreLineare r(GetN());
	for (unsigned int i = 0; i<GetN(); i++)
		 r.SetComponent(i, GetComponent(i)*a);
	return r;
}


void VettoreLineare:: Stampa(){
	int width = int(log10(m_N)+1);	
